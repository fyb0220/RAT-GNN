...
from RATGNN import RATGNN
sys.path.append('..')
from utils import *
from surrogate_model.gcn import GCN
from surrogate_model.sage import GraphSAGE
from surrogate_model.gat import GAT, LayerType
from glove_utils import GloveModel
...
def main(opts):
    ...
    # Preprocessing data
    adj, features, labels_np = load_npz(opts['dataset'])
    n = adj.shape[0]
    nc = labels_np.max()+1
    ...
    feat_num = int((features != 0).sum(1).float().mean())
    ...
    # Surrogate model
    if surro_type == 'gcn':
        surro_net = GCN(features.shape[1], 128, labels.max().item() + 1, 0.5).float().to(device)
        ...
        surro_net = GraphSAGE(features.shape[1], 128, labels.max().item() + 1, 0.5).float().to(device)
    surro_net.load_state_dict(torch.load(surro_save_file+'_checkpoint.pt'))


    # Evalution model
    if victim_type == 'gcn':
        victim_net = GCN(features.shape[1], 128, labels.max().item() + 1, 0.5).float().to(device)
    elif victim_type == 'gat':
        victim_net = GAT(num_of_layers=2, num_heads_per_layer=[nhead, 1], num_features_per_layer=[nfeat, nhid, nc],
            add_skip_connection=False, bias=True, dropout=0.6,
            layer_type=LayerType.IMP2, log_attention_weights=False).to(device)
    else:
        victim_net = GraphSAGE(features.shape[1], 128, labels.max().item() + 1, 0.5).float().to(device)
    ...
    # Initialization
    model = RATGNN(labels, nfeat, W1, W2, discrete, device, feat_min=feat_min, feat_max=feat_max, feat_num=feat_num).to(device)
    ...
    for epoch in range(st_epoch, nepochs):
        ...
        for batch_x, _ in batch_loader:
            ...
            train_loss = torch.stack(loss_arr).sum()
            optimizer.zero_grad()
            train_loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--surro_type', default='sage',help='surrogate gnn model')
    parser.add_argument('--victim_type', default='sage',help='victim gnn model')
    ...
    GAT_para = {'politifact':(2,2), 'gossipcop':(2,2)}
