...
def load_npz(file_name):
    ...
    with np.aload('datasets/' + file_name) as loader:
        ...
        node_attributes = np.load('datasets/' + 'pol_glove_feature.npy')
        attr_matrix = torch.from_numpy(node_attributes).to(torch.float)
        labels = loader.get('labels')
        num_nodes = attr_matrix.size(0)
        labels = torch.LongTensor(labels)
        if labels.size(0) < num_nodes:
            labels = torch.cat((labels, torch.zeros(num_nodes - labels.size(0), dtype=torch.long)))
...
def load_npz2(file_name):
    ...
    with np.aload('datasets/' + file_name) as loader:
        ...
        adj_matrix = sp.csr_matrix((loader['adj_data'], loader['adj_indices'],
                                    loader['adj_indptr']), shape=loader['adj_shape'])
        node_attributes = np.load('datasets/' + 'glove_feature.npy')
        attr_matrix = torch.from_numpy(node_attributes).to(torch.float)
        labels = loader.get('labels')
...
def read_graph_data(folder):
    # Load node attributes (features)
    node_attributes = np.load(folder + 'glove_feature.npy')

    # Load the adjacency matrix and labels from .mat file
    ...
    # Pad labels if necessary (assuming y might have fewer labels than nodes)
